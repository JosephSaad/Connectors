...
// env/, logs/ and data/ against the current directory.  SEISMIC_CONNECTOR_HOME (or,
...
public static class ServiceHost
{
    ...
    {
        var home = Environment.GetEnvironmentVariable("SEISMIC_CONNECTOR_HOME");
        Directory.SetCurrentDirectory(
            !string.IsNullOrWhiteSpace(home) ? home : AppContext.BaseDirectory);

        var builder = Host.CreateApplicationBuilder();
        builder.Services.AddWindowsService(
            options => options.ServiceName = "SeismicConnector");
    ...
    private sealed class CommandWorker : BackgroundService
    {
        private static readonly IAppLogger Logger =
            Logging.GetLogger("seismic_connector.service");
