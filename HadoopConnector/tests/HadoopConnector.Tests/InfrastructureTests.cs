...
public class MetricsTests
{
    ...
    public void Counters_AccumulateAndRender()
    {
        ...
        Metrics.IncHdfsCalls(7);
        Metrics.IncPartitionsScanned(4);
        Metrics.IncPartitionsPruned(11);
        Metrics.IncRecordsScanned(100);
        ...
        Assert.Equal(4, Metrics.PartitionsScanned);
        Assert.Equal(11, Metrics.PartitionsPruned);
        Assert.Equal(100, Metrics.RecordsScanned);
        Assert.Equal(60, Metrics.RecordsMatched);
        Assert.Equal(40, Metrics.RecordsFilteredFor("predicate"));
        Assert.Equal(0, Metrics.RecordsFilteredFor("partition"));

        var text = Metrics.RenderPrometheus();
        Assert.Contains("# TYPE hadoop_connector_items_ingested_total counter", text);
        Assert.Contains("hadoop_connector_items_ingested_total 5", text);
        Assert.Contains("hadoop_connector_throttled_429_total 3", text);
        Assert.Contains("hadoop_connector_hdfs_calls_total 7", text);
        Assert.Contains("hadoop_connector_partitions_scanned_total 4", text);
        Assert.Contains("hadoop_connector_partitions_pruned_total 11", text);
        Assert.Contains("hadoop_connector_records_scanned_total 100", text);
        Assert.Contains("hadoop_connector_records_matched_total 60", text);
...
public class SecretProviderTests : IDisposable
{
    ...
    {
        Assert.Equal(
            "secret-hdfs-delegation-token", SecretProvider.ToSecretName("SECRET_HDFS_DELEGATION_TOKEN"));
    }
