...
/// <summary>
/// Source-hardening pins: id hygiene end-to-end. The Graph external item id is
/// the raw Salesforce record id, so a poisoned/absent id column must be caught
/// before it can corrupt the deletion sweep or escape into request paths.
/// </summary>
public class SourceHardeningTests
{
    [Fact]
    public void ValidateSourceRecordIds_AllRowsIdLess_IsHardError()
    {
        // A fetch where EVERY predicate-matched row lacked an id is an export
        // with a missing/renamed Id column — refusing beats a silently-empty
        // crawl that would sweep the whole inventory.
        var result = new HadoopConnector.Hdfs.FetchResult
        {
            Stats = { RecordsScanned = 5, RecordsDroppedNoId = 5, RecordsMatched = 0 },
        };
        var exc = Assert.Throws<InvalidDataException>(
            () => IngestPipeline.ValidateSourceRecordIds("Contact", result));
        Assert.Contains("Contact", exc.Message);
        Assert.Contains("id column", exc.Message, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void ValidateSourceRecordIds_PartialOrCleanFetches_Pass()
    {
        // Individual id-less rows alongside real records are dropped, not fatal.
        var partial = new HadoopConnector.Hdfs.FetchResult
        {
            Records =
            {
                new HadoopConnector.Hdfs.BdhRecord(
                    "Contact", new JsonObject { ["Id"] = "0035e00000abcde" }),
            },
            Stats = { RecordsDroppedNoId = 3, RecordsMatched = 1 },
        };
        IngestPipeline.ValidateSourceRecordIds("Contact", partial);

        // A genuinely-empty fetch (nothing matched, nothing dropped) is fine too.
        IngestPipeline.ValidateSourceRecordIds(
            "Contact", new HadoopConnector.Hdfs.FetchResult());
    }

    [Theory]
    [InlineData("0035e00000abcde", true)]
    [InlineData("005000000000001AAA", true)]   // 18-char Salesforce id
    [InlineData("A_b-C", true)]
    [InlineData("", false)]
    [InlineData("../etc/passwd", false)]       // traversal shape
    [InlineData("a/b", false)]
    [InlineData("has space", false)]
    [InlineData("semi;colon", false)]
    public void IsSafeItemId_AcceptsGraphSafeTokens_Only(string id, bool expected)
    {
        Assert.Equal(expected, HadoopConnector.Hdfs.BdhRecord.IsSafeItemId(id));
    }

    [Fact]
    public void IsSafeItemId_RejectsOverlongIds()
    {
        Assert.True(HadoopConnector.Hdfs.BdhRecord.IsSafeItemId(new string('a', 128)));
        Assert.False(HadoopConnector.Hdfs.BdhRecord.IsSafeItemId(new string('a', 129)));
    }

    [Fact]
    public void LocalPathSource_Resolve_RefusesTraversalEscapes()
    {
        using var dir = new TempDir();
        var source = new HadoopConnector.Hdfs.LocalPathSource(dir.Path);

        Assert.Throws<HadoopConnector.Hdfs.HdfsException>(() => source.Resolve("../../etc"));
        Assert.Throws<HadoopConnector.Hdfs.HdfsException>(
            () => source.Resolve("Contact/../../../etc/passwd"));

        // Normal relative paths resolve under the export root.
        var resolved = source.Resolve("Contact/dt=2026-01-01");
        Assert.StartsWith(dir.Path, resolved, StringComparison.Ordinal);
    }
}
...
public class LogGatingTests : IDisposable
{
    ...
    public void IsEnabledFor_NoFileSink_FollowsConsoleThreshold()
    {
        ...
        var logger = Logging.GetLogger("hadoop_connector.test");
