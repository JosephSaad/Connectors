...
public class ShardingConfigTests
{
    ...
    public void ValidExactPartition_Parses()
    {
        using var scope = new EnvScope((ShardingConfig.EnvVar,
            """{"bdhMartA": ["Contact", "Account"], "bdhMartB": ["Lead"]}"""));
        Assert.True(ShardingConfig.TryLoad(
            Schema("Contact", "Account", "Lead"), out var shards, out var error));
        Assert.Null(error);
        Assert.Equal(2, shards.Count);
        Assert.Equal("bdhMartA", shards[0].ConnectionId);
        Assert.Equal(new[] { "Contact", "Account" }, shards[0].ObjectTypes);
        Assert.Equal(new[] { "Lead" }, shards[1].ObjectTypes);
    }
    ...
    public void CloneForConnection_SwapsOnlyTheConnectorId()
    {
        ...
        Assert.Equal(baseConfig.HdfsNamenodeUrl, clone.HdfsNamenodeUrl);
        Assert.Equal(baseConfig.BdhRootPath, clone.BdhRootPath);
        Assert.Equal(baseConfig.BdhLagHours, clone.BdhLagHours);
