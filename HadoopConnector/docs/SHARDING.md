...
per connection), so spreading the BDH object types across **N**
...
Each **shard** is one Graph external connection paired with the set of BDH
...
3. All state is per shard connection id: checkpoints
   ...
   sync timestamps, item inventories, and — in HA — crawl/claim rows. The
   health endpoint's dead-letter depth is the sum across shards.
...
GRAPH_CONNECTION_SHARDS='{
  "bdhCrmCore": ["Contact", "Account"],
  "bdhCrmPipeline": ["Opportunity", "Case", "Lead"]
}'
```

Shard connection ids are full connector ids in their own right (same naming
rules as `CONNECTOR_ID`) — keep them distinct from the live Salesforce
connector's connections too, since both connectors emit the same Salesforce
record ids (README warning).
...
- Treat "N shards ≈ N × 25 items/s" as a planning assumption; validate against
  ...
  concurrency dial and `hadoop_connector_throttled_429_total` metric show
