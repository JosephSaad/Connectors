...
| `dbo.Principals` | `data/{id}_identity.db` (SQLite) | `(ConnectorId, SourceId)` |
...
- **Principals:** MERGE upsert keyed on `(ConnectorId, SourceId)` — `SourceId`
  is the Salesforce user id from the BDH `User` export; the secondary index
  `IX_Principals_Email` serves owner-email fallback resolution.
...
- **CrawlRuns:** `Status` transitions `open → closed` (or `open → failed` when
  ...
  the sync-watermark stamp exactly-once.
...
The app needs `SELECT/INSERT/UPDATE/DELETE` on the seven tables, `SELECT` on
the view, and `EXECUTE` on `usp_PruneHistory` — nothing else (no DDL after
provisioning).
