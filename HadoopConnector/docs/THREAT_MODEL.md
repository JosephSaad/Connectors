...
| STRIDE | Threat | Mitigation | Residual |
|---|---|---|---|
| I | Second copy of business data accumulating outside the index's ACLs | `DEADLETTER_PAYLOAD_MODE=redacted` strips record values pre-write (ids, object type, error, names, sizes, SHA-256 hashes kept — `Config/DeadLetterRedaction.cs`); unknown mode values fail TOWARD redaction; retry-failed re-fetches from BDH so redaction costs no capability (incl. the oversize-inconclusive keep rule) | Default remains `full` for parity/diagnosis — flip to `redacted` where the queue's storage is less protected than the source |
