...
for a run — so a single crawl is followable end-to-end.
...
One `ActivitySource` named **`HadoopConnector`** (`Infrastructure/Tracing.cs`).
Span tree per crawl cycle — every tag is prefixed `bdh.`:

```
crawl.cycle                       (bdh.crawl.kind = full|incremental,
                                   bdh.connector.id, bdh.correlation_id)
└─ crawl.object                   (bdh.object_type, bdh.connector.id, bdh.correlation_id)
   ├─ source.fetch                (bdh.object_type, bdh.source = bdh,
   │                               bdh.records_scanned, bdh.records_matched,
   │                               bdh.partitions_pruned)
   ├─ transform.chunk             (bdh.object_type, bdh.record_count, bdh.item_count)
   ├─ graph.ingest                (bdh.object_type, bdh.item_count)
   └─ deletion.sweep              (bdh.object_type, bdh.stale_count)   [full crawls]
```

Spans are parented by `Activity.Current`, so the tree reflects the call graph
(one `crawl.object` per object type; `transform.chunk`/`graph.ingest` repeat
per chunk). The `source.fetch` span covers the whole filtered fetch —
partition scan, streamed parsing, record predicates and row cap — and carries
the filter layer's per-stage counts as tags (`docs/FILTERS.md`). Every span
carries the ambient `bdh.correlation_id` tag.

## Correlation id

- One id per **crawl cycle** (a `RunAsync` call).
...
- `validate-config` prints the exporter target (or "disabled") — informational,
  never a failure.
- `/metrics` includes `hadoop_connector_tracing_enabled` (gauge: 1 when the
