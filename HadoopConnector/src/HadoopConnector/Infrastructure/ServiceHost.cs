...
// env/, logs/ and data/ against the current directory. HADOOP_CONNECTOR_HOME
...
public static class ServiceHost
{
    ...
    {
        var home = Environment.GetEnvironmentVariable("HADOOP_CONNECTOR_HOME");
        Directory.SetCurrentDirectory(
            !string.IsNullOrWhiteSpace(home) ? home : AppContext.BaseDirectory);

        var builder = Host.CreateApplicationBuilder();
        builder.Services.AddWindowsService(options => options.ServiceName = "HadoopConnector");
    ...
    {
        private static readonly IAppLogger Logger = Logging.GetLogger("hadoop_connector.service");
