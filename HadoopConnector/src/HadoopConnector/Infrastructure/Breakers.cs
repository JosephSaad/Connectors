...
// (the BDH/HDFS source client and the Microsoft Graph client). Both are
...
{
    /// <summary>Breaker guarding the BDH source client (WebHDFS/localpath).</summary>
    public static CircuitBreaker Hdfs { get; private set; } = CircuitBreaker.Disabled;
    ...
    {
        Hdfs = new CircuitBreaker("hdfs", options, now);
    ...
    public static bool IsAnyCriticalOpen => Hdfs.IsOpen || Graph.IsOpen;
        ...
        StringBuilder sb, string name, string type, string help, string label, string value)
    {
        ...
        // dependency (hdfs) so the exposition stays valid (no duplicates).
        if (label.Contains("\"hdfs\""))
        {
