...
public sealed class ExternalItem
{
    ...
    /// <summary>Optional index self-expiry (GRAPH_ITEM_TTL_DAYS): when set, the
    /// externalItem carries <c>expirationDateTime</c> so the index drops stale
    /// items if crawling stops. Null (default) = no expiry.</summary>
