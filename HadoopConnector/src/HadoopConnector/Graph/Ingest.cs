// Graph/Ingest.cs
// ---------------
// The ingestion pipeline: BDH records → ACL resolution → externalItem
...
//   1. Fetch records via BdhFetcher — fail-closed full-scan guard, partition
//      pruning (dt watermark − BDH_LAG_HOURS overlap + partition filters),
//      streamed parsing, record predicates and the row-cap safety valve.
...
// close-with-failed-claims semantics. A FullScanRefusedException (the scale
// guard) is handled the same way minus the dead-letter noise: the object is
// recorded as failed and the crawl moves on.
...
public sealed class IngestSummary
{
    ...
    /// <summary>Object types truncated by the BDH_MAX_RECORDS_PER_OBJECT row cap
    /// (partial crawl: alerted, deletion sweep skipped).</summary>
    public List<string> PartialObjects { get; } = new();
...
{
    private static readonly IAppLogger Logger = Logging.GetLogger("hadoop_connector.ingest");
...
public sealed class IngestPipeline
{
        ...
        bool fullCrawl, IReadOnlyList<string>? onlyObjects = null, CancellationToken ct = default)
    {
        ...
        cycleSpan?.SetTag("bdh.correlation_id", CorrelationContext.Current);
        ...
        var finished = !summary.Stopped && !summary.Degraded;
        if (finished)
        {
    ...
    /// CLASSIFICATION is off). Called after conversion so the full content
        ...
        foreach (var objectConfig in objects)
        {
            ...
            try
            {
                ...
                await IngestObjectAsync(
                    ...
                    .ConfigureAwait(false);
                if (_ha is not null && crawlKey is not null
                    && !summary.Stopped && !summary.Degraded)
                {
                    _ha.CompleteObject(crawlKey, objectConfig.ObjectName);
                }
            }
            catch (FullScanRefusedException exc)
            {
                // Fail-closed scale guard: refuse the unfiltered object, keep
                // crawling the rest. validate-config --strict catches this
                // before deployment; at runtime it is a per-object failure.
    ...
    internal async Task<FetchResult> FetchRecordsAsync(
        ObjectConfig objectConfig, bool fullCrawl, DateTime? since, CancellationToken ct)
    {
        var result = await _fetcher.FetchAsync(objectConfig, fullCrawl, since, ct)
            .ConfigureAwait(false);
        ValidateSourceRecordIds(objectConfig.ObjectName, result);
        return result;
    }

    /// <summary>
    /// Reject a fetch whose rows carried NO usable ids at all. The fetcher
    /// already drops individual id-less rows (they would corrupt the deletion
    /// sweep — every real id would look stale); a batch where EVERY predicate-
    /// matched row lacked an id is an export with a missing/renamed Id column
    /// and is a hard error instead of a silently-empty crawl that would sweep
    /// the whole inventory.
    /// </summary>
    internal static void ValidateSourceRecordIds(string objectName, FetchResult result)
    {
        var stats = result.Stats;
        if (result.Records.Count == 0 && stats.RecordsDroppedNoId > 0 && stats.RecordsMatched == 0)
        {
            throw new InvalidDataException(
                $"{objectName}: the BDH export has no recognized id column — all "
                + $"{stats.RecordsDroppedNoId} predicate-matched row(s) lack an 'Id' value. "
                + "Refusing to ingest: an id-less crawl would corrupt deletion sync. Fix the "
                + "export to include the Salesforce Id column and re-run.");
        }
    }
        ...
        CancellationToken ct)
    {
        ...
        sweepSpan?.SetTag("bdh.stale_count", stale.Count);
        ...
        Logger.Info(
            $"{objectConfig.ObjectName}: deleting {stale.Count} item(s) no longer present in BDH...");
        ...
        CancellationToken ct)
    {
        ...
        transformSpan?.SetTag("bdh.item_count", items.Count);
    ...
    public async Task<(bool Ok, string? Error)> IngestSingleAsync(
        BdhRecord record, ObjectConfig objectConfig, CancellationToken ct = default)
    {
        var acl = _aclResolver.Resolve(record, objectConfig);
        if (acl.Count == 0)
    ...
    public async Task<(bool Ok, string? Error)> WithdrawSingleAsync(
        ObjectConfig objectConfig, string recordId, CancellationToken ct = default)
    {
        var itemId = recordId;
        var cfg = _config;
        if (ShardingConfig.TryLoad(_schema, out var shards, out _))
