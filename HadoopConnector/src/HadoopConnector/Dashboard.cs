    ...
    public static void CrawlSummary(string kind, IngestSummary summary)
    {
        lock (Sync)
        {
            if (PlainMode)
            {
                ...
                foreach (var objectType in summary.PartialObjects)
                    Console.WriteLine($"  PARTIAL (row cap hit): {objectType}");
                return;
            }
            ...
            {
                AnsiConsole.MarkupLine(
                    $"[yellow]PARTIAL (row cap hit): {Markup.Escape(objectType)} — tighten filters.json[/]");
            }
        }
    }
}
