...
(`hadoop-connector.cdx.json`), generated by `.github/workflows/release-hadoop.yml`
and attached to each `hadoop-v*` release. The CodeQL security-and-quality scan
runs from `.github/workflows/codeql.yml`. Both live at the repository root,
which is where GitHub executes workflows from.
