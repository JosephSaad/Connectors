...
public class HaCoordinator
{
    ...
    /// (including steals). The host wires it to its own metrics facade (e.g.
    /// <c>() =&gt; { Metrics.IncHaClaimsAcquired(); Metrics.AddHaClaimsHeld(1); }</c>),
    /// so the chassis records HA-claim metrics without referencing a connector
    /// type. Null → no metric side effect.
    /// </summary>
    public static Action? ClaimAcquiredHook;

    /// <summary>
    /// Optional host hook invoked once each time THIS node releases a held HA
    /// claim (a claim it owned transitions to done|failed). The host wires it to
    /// its metrics facade (e.g. <c>() =&gt; Metrics.AddHaClaimsHeld(-1)</c>).
    /// Null → no metric side effect.
    /// </summary>
    public static Action? ClaimReleasedHook;
