...
//     listens). A single ActivitySource (the host's Event Log source) is used to
//     open spans for the meaningful stages (crawl cycle, teamsite crawl, Seismic
...
//
//  2. OTLP export (opt-in, gated on ChassisTracingOptions.Enabled — the host
//     sets it from OTEL_EXPORTER_OTLP_ENDPOINT). Only when an endpoint is
//     supplied is a TracerProvider built and a listener registered; the OTLP
//     exporter uses a batched, bounded background queue, so a broken/unreachable
//     collector can never fail or stall a crawl (export is fire-and-forget on a
//     background thread; drops are silent). With no endpoint, nothing is
//     registered and overhead is unchanged.
//
// The host resolves the standard OTEL_* env vars (endpoint, protocol, headers)
// and the service name and passes them in via ChassisTracingOptions, so the
// chassis stays connector-neutral (it reads no host config type).
...
/// <summary>
/// Connector-neutral tracing configuration the host supplies to
/// <see cref="Tracing.Initialize"/>. The host reads the standard OTEL_* env vars
/// and its own config to populate these, so the chassis never touches a host
/// config type.
/// </summary>
public sealed record ChassisTracingOptions(
    bool Enabled, string? OtlpEndpoint, string? OtlpProtocol, string? OtlpHeaders, string ServiceName);
...
public static class Tracing
{
    ...
    /// connector hardcoded).</summary>
    public static string ActivitySourceName => Chassis.Identity.EventLogSource;
    ...
    /// <summary>
    /// Build the OTLP TracerProvider IF <paramref name="options"/> carries an
    /// endpoint (<c>Enabled</c>); otherwise leave tracing inert (no listener,
    /// unchanged overhead). Idempotent and never throws — a misconfigured
    /// exporter must not stop the connector from running.
    /// </summary>
    public static void Initialize(ChassisTracingOptions options)
    {
        ...
        try
        {
            _provider = Sdk.CreateTracerProviderBuilder()
                ...
                .AddOtlpExporter(exporterOptions =>
                {
                    ...
                    exporterOptions.Protocol = ResolveProtocol(options.OtlpProtocol);
    ...
    private static OtlpExportProtocol ResolveProtocol(string? raw)
    {
        var value = (raw ?? "").Trim().ToLowerInvariant();
        return value is "grpc" ? OtlpExportProtocol.Grpc : OtlpExportProtocol.HttpProtobuf;
    }
