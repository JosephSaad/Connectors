<#
.SYNOPSIS
    Installs (or removes) the Clarizen Copilot Connector as a Windows service.
...
.DESCRIPTION
    ...
    Deployment layout expected in -InstallDir (see README "Windows service"):
        ClarizenConnector.exe    (dotnet publish output, .NET 10 / win-x64)
        config\    schema.json, graph-schema.json
        env\       .env.local, .env.local.user
    logs\ and data\ are created there at runtime (CLARIZEN_CONNECTOR_HOME).
...
.EXAMPLE
    ...
    .\scripts\install-windows-service.ps1 -InstallDir "C:\ClarizenConnector" `
        ...
        -ServiceName "ClarizenCopilotConnector"

.EXAMPLE
    .\scripts\install-windows-service.ps1 -ServiceName "ClarizenConnector" -Uninstall
#>
[CmdletBinding()]
param(
    [string]$ServiceName = "ClarizenConnector",
    [string]$DisplayName = "Clarizen Copilot Connector",
    [string]$InstallDir,
...
$exe = Join-Path $InstallDir "ClarizenConnector.exe"
...
if ($existing) {
    ...
    New-Service -Name $ServiceName -DisplayName $DisplayName `
        ...
        -Description "Ingests Clarizen (Planview AdaptiveWork) work items into Microsoft 365 Copilot / Microsoft Search." | Out-Null
}
