...
2. **Claim** — each Clarizen object type is an atomic work item in
...
5. **Close** — when every object type is `completed`, the nodes race a guarded
   ...
   delta-sync timestamp. Losers skip the stamp, so the cursor is written once.
...
- [ ] Remember the Clarizen API budget is **per org**: divide
      `CLARIZEN_API_CALLS_PER_DAY` by the node count so the fleet stays under
      the org quota.
...
| All nodes die | The crawl stays open; the next scheduled slot opens a new crawl and the checkpoint/delta cursor guarantee no loss. |
