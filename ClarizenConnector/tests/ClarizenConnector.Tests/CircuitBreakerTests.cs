...
public class BreakersRegistryTests : IDisposable
{
    ...
    public void Default_IsInertPassthrough()
    {
        ...
        Assert.False(Breakers.Clarizen.Enabled);
    ...
    {
        Breakers.Initialize(new CircuitBreakerOptions { Enabled = true, FailureThreshold = 1 });
        Breakers.Clarizen.TripForTests();

        var text = Metrics.RenderPrometheus();
        Assert.Contains("clarizen_connector_circuit_breaker_state{dependency=\"clarizen\"} 2", text);
        Assert.Contains("clarizen_connector_circuit_breaker_state{dependency=\"graph\"} 0", text);
        Assert.Contains("clarizen_connector_circuit_breaker_trips_total{dependency=\"clarizen\"} 1", text);
        Assert.Contains("# TYPE clarizen_connector_circuit_breaker_state gauge", text);
        Breakers.ResetForTests();
    }
}
