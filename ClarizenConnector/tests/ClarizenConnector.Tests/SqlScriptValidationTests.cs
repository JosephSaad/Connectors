...
public class SqlScriptValidationTests
{
    ...
    private static string RepoRoot()
    {
        ...
        while (dir != null && dir.GetFiles("ClarizenConnector.sln").Length == 0)
            dir = dir.Parent;
        Assert.True(dir != null,
            "could not locate repo root (ClarizenConnector.sln) above " + AppContext.BaseDirectory);
        return dir!.FullName;
    }

    private static string ScriptPath => Path.Combine(RepoRoot(), "scripts", "sql", "create-database.sql");
    private static string SrcRoot => Path.Combine(RepoRoot(), "src", "ClarizenConnector");
