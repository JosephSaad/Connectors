...
// --------------------------
// `identity-dry-run [--save]` — run the Clarizen → Entra identity resolution
    ...
    public static async Task<object?> RunAsync(ParsedArgs args)
    {
        ...
        try
        {
            ...
            var result = await context.IdentitySync.SyncAsync(
                snapshot, context.IdentityStore, persist: save, ServiceStop.Token);
            ...
            Dashboard.Line($"Groups:              {result.GroupsTotal}");
            Dashboard.Line($"Projects w/ members: {result.ProjectsWithMembers}");
