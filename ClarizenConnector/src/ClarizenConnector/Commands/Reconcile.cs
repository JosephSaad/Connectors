...
//
//   MISSING (in Clarizen, not indexed) — reported; the next crawl or
//       `retry-failed` ingests these.
//   STALE (indexed, gone from Clarizen) — reported; with --fix the items are
    ...
    public static async Task<object?> RunAsync(ParsedArgs args)
    {
        ...
        try
        {
            var reconciler = new Reconciler(
                context.Config, context.Schema, context.Clarizen, context.Graph);
