...
public static class Deploy
{
    ...
    private static async Task<object?> RunAsync(ParsedArgs args, bool provision, string runPrefix)
    {
        ...
        try
        {
            ...
            // Event-driven incremental (optional, off unless CLARIZEN_WEBHOOK_PORT
            // + CLARIZEN_WEBHOOK_SECRET are set). The polling cursor above stays
            // the default and the fallback whenever the receiver is off or down.
            using var webhook = await StartWebhookIfConfiguredAsync(context);
    ...
    /// <summary>
    /// Start the webhook receiver + processor if configured. Returns an
    /// IDisposable that stops both; a no-op disposable when webhooks are off
    /// or fail-closed (missing secret). The processor reuses the pipeline
    /// built from the just-completed full crawl's identity snapshot; the
    /// polling cursor remains the correctness backstop.
    /// </summary>
    private static async Task<IDisposable> StartWebhookIfConfiguredAsync(RuntimeContext context)
    {
        var portRaw = Environment.GetEnvironmentVariable(WebhookReceiver.PortEnvVar);
        if (string.IsNullOrWhiteSpace(portRaw))
            return NoopDisposable.Instance;

        var pipeline = await context.BuildPipelineAsync(ServiceStop.Token);
        var processor = new WebhookProcessor(context.Schema, context.Clarizen, pipeline);
        var receiver = WebhookReceiver.StartIfConfigured(processor);
        if (receiver is null)
            return NoopDisposable.Instance;

        var worker = Task.Run(() => processor.RunAsync(ServiceStop.Token));
        Dashboard.Line("Event-driven incremental active: webhook receiver + processor running.");
        return new WebhookLifetime(receiver, worker);
    }

    private sealed class WebhookLifetime : IDisposable
    {
        private readonly WebhookReceiver _receiver;
        private readonly Task _worker;

        public WebhookLifetime(WebhookReceiver receiver, Task worker)
        {
            _receiver = receiver;
            _worker = worker;
        }
        ...
        public void Dispose()
        {
            ...
            try
            {
                // ServiceStop cancellation ends the worker; give it a moment to
                // flush pending events, then move on.
    ...
    internal static async Task<CycleResult> RunCycleAsync(RuntimeContext context, bool fullCrawl)
    {
        ...
        try
        {
            ...
            {
                var snapshot = await context.GetSnapshotAsync(ServiceStop.Token);
                await context.IdentitySync.SyncAsync(
                    snapshot, context.IdentityStore, persist: true, ServiceStop.Token);
            }
