// Webhook/SignatureValidator.cs
// -----------------------------
// Shared-secret HMAC validation for inbound webhook posts. This is the
// security boundary: NO payload is parsed or enqueued until its signature is
// validated against CLARIZEN_WEBHOOK_SECRET.
//
// Contract:
//   • The sender computes HMAC-SHA256(secret, raw-request-body) and sends it
//     in the signature header (default X-Clarizen-Signature). Hex (optionally
...
//   • Fail-closed: a missing/empty secret means the receiver must not run at
//     all (enforced by the receiver); a missing/malformed/mismatched signature
//     on a request is rejected.
...
public sealed class SignatureValidator
{
    ...
    public const string DefaultHeader = "X-Clarizen-Signature";
    ...
    /// Constant-time in the length-matched path.
    /// </summary>
    ...
    internal static byte[]? TryDecode(string signature)
    {
        ...
        try
        {
            if (signature.Length % 2 == 0
                && signature.All(c => Uri.IsHexDigit(c)))
            {
