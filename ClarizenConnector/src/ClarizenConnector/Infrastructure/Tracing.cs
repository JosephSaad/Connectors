...
//
// The connector owns one ActivitySource ("ClarizenConnector"). Pipeline stages
// (crawl cycle, per-object crawl, source fetch, transform, Graph batch ingest,
// deletion sweep, webhook event) open Activities on it, parented by the
...
{
    private static readonly IAppLogger Logger = Logging.GetLogger("clarizen_connector.tracing");

    /// <summary>The single ActivitySource all connector spans are emitted on.</summary>
    public const string SourceName = "ClarizenConnector";
    ...
    public static Activity? StartCrawlCycle(string kind, string connectorId)
    {
        ...
        span?.SetTag("clarizen.crawl.kind", kind);
        span?.SetTag("clarizen.connector.id", connectorId);
        return span;
    }
    ...
    public static Activity? StartObjectCrawl(string objectType, string connectorId)
    {
        ...
        span?.SetTag("clarizen.object_type", objectType);
        span?.SetTag("clarizen.connector.id", connectorId);
        Stamp(span);
        return span;
    }

    /// <summary>Span for a source fetch (Clarizen REST or TDW bulk).</summary>
    public static Activity? StartSourceFetch(string objectType, string source)
    {
        ...
        span?.SetTag("clarizen.object_type", objectType);
        span?.SetTag("clarizen.source", source);  // "rest" | "tdw"
        Stamp(span);
        return span;
    }
    ...
    public static Activity? StartTransform(string objectType, int recordCount)
    {
        ...
        span?.SetTag("clarizen.object_type", objectType);
        span?.SetTag("clarizen.record_count", recordCount);
        Stamp(span);
        return span;
    }
    ...
    public static Activity? StartGraphIngest(string objectType, int itemCount)
    {
        ...
        span?.SetTag("clarizen.object_type", objectType);
        span?.SetTag("clarizen.item_count", itemCount);
        Stamp(span);
        return span;
    }
    ...
    /// <summary>Root span for handling one webhook event.</summary>
    public static Activity? StartWebhookEvent(string objectType, string kind)
    {
        var span = Source.StartActivity("webhook.event", ActivityKind.Consumer);
        span?.SetTag("clarizen.object_type", objectType);
        span?.SetTag("clarizen.change_kind", kind);
        Stamp(span);
        return span;
    }
    ...
    private static void Stamp(Activity? span)
    {
        if (span is not null && CorrelationContext.Current is { } id)
            span.SetTag("clarizen.correlation_id", id);
    }
