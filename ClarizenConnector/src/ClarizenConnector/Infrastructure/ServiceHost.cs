...
//                pipeline stops at the next chunk boundary (pending Graph
//                batch flushed, checkpoint saved) and the command returns.
...
// env/, logs/ and data/ against the current directory. CLARIZEN_CONNECTOR_HOME
// (or, failing that, the executable's directory) is used as the working
// directory.
...
public static class ServiceHost
{
    ...
    {
        var home = Environment.GetEnvironmentVariable("CLARIZEN_CONNECTOR_HOME");
        Directory.SetCurrentDirectory(
            !string.IsNullOrWhiteSpace(home) ? home : AppContext.BaseDirectory);

        var builder = Host.CreateApplicationBuilder();
        builder.Services.AddWindowsService(options => options.ServiceName = "ClarizenConnector");
        builder.Services.Configure<HostOptions>(options => options.ShutdownTimeout = ShutdownTimeout);
        builder.Services.AddHostedService(
            sp => new CommandWorker(args, executeCommand, sp.GetRequiredService<IHostApplicationLifetime>()));
    ...
    {
        private static readonly IAppLogger Logger = Logging.GetLogger("clarizen_connector.service");
        ...
        public CommandWorker(
            string[] args, Func<string[], Task<int>> executeCommand, IHostApplicationLifetime lifetime)
        {
            _args = args;
