...
// -----------------------------
// SQL Server access helper for the optional SQL state backend (USE_SQL_SERVER)
// and HA coordination. Wraps Microsoft.Data.SqlClient with:
//
//   • Encrypt=True forced unless the connection string already sets Encrypt.
//   • Optional Entra Managed Identity auth (SQL_USE_MANAGED_IDENTITY=true) —
//     an access token for https://database.windows.net/.default is attached
...
//
// ISqlGateway is the seam the state store and HA coordinator run against;
...
public sealed class SqlExecutor : ISqlGateway
{
    private static readonly IAppLogger Logger = Logging.GetLogger("clarizen_connector.sql");
    ...
    public SqlExecutor(string? connectionString = null)
    {
