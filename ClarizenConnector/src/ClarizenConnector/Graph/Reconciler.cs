...
//
//   source    — the live Clarizen record set (TDW bulk export when available,
//               REST API otherwise; the same fetch path the crawler uses),
...
//   STALE   — in the inventory but gone from the source (deleted in
//             Clarizen). `reconcile --fix` DELETEs these from the Graph
...
{
    private static readonly IAppLogger Logger = Logging.GetLogger("clarizen_connector.reconcile");
        ...
        string? onlyType = null, bool fix = false, CancellationToken ct = default)
    {
        ...
        foreach (var objectConfig in _schema.ObjectList)
        {
            ...
            var source = await IngestPipeline.FetchSourceRecordsAsync(
                    _config, _clarizen, objectConfig, fullCrawl: true, since: null, ct)
                .ConfigureAwait(false);
            var sourceIds = source.Select(r => r.ItemId).ToList();
