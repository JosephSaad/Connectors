...
public static class FeedReader
{
    ...
    /// <summary>Parse a data file into records. JSON (.json/.jsonl) or CSV (.csv).</summary>
    public static IReadOnlyList<FeedRecord> ReadRecords(string filePath, string dataset)
    {
        ...
        {
            ".json" => ParseJson(File.ReadAllText(filePath), canonical),
            ".jsonl" => ParseJsonLines(File.ReadLines(filePath), canonical),
            ".csv" => ParseCsv(File.ReadLines(filePath), canonical),
            _ => throw new NotSupportedException($"Unsupported feed file type '{extension}' ({filePath})"),
        };
    }
